...
system('echo "" > ./multi-searchnpc.log');

foreach my $in (@$npclist){
   #    print Dumper($in);

   system("./searchnpc_n.pl $in->{email} $in->{emailpass} $in->{lat} $in->{lng} $in->{runmode} >> ./multi-searchnpc.log 2>&1 &");
...
#   system("./searchnpc.pl $npclist->[$count]->{email} $npclist->[$count]->{emailpass} $npclist->[$count]->{lat} $npclist->[$count]->{lng} $npclist->[$count]->{runmode} >> ./multi-searchnpc.log 2>&1 &");
#   sleep 1;
#}
